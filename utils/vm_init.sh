...
npm install
# If all dependencies are already installed we could avoid npm install 
...
VM_IP=$(sudo npx ts-node ./virtualmachine/idm_test_workflow.ts)
echo "IDM_SYSTEM=${VM_IP}"
